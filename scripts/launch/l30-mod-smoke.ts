...
const parent = path.join(root, 'tmp/l30-mod-smoke')
...
async function main() {
    ...
    const { pinDatabase } = require('../../electron/mod-assets')
    const payload = await pinDatabase(path.dirname(file), directory)
    ...
    for (const player of payload.players) {
        ...
        assert.equal(loaded.portraitPath, player.portraitPath || '')
    }
    const career = loader.createCareerFromSnapshot('Isolated L30 mod verification', payload.teams[0].id)!
