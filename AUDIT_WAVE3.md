...
| ✅ | ~~AI roster churn is silent~~ | DONE 2026-06-18: re-verified that AI-to-AI **transfers** already surface in the news feed (capped 50) and AI **bids on the player's listings** hit the event log. The gap was free-agent signings — now MARQUEE signings (skill ≥ 78) also hit the news feed (`signFreeAgent`), while routine depth-filling stays out by design (the very log-noise this item warned about). Tested. |
