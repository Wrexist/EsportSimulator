...
team reputation (1×→2×) so they feel earned by climbing rather than flat weekly RNG.)* *(B4 shipped: dashboard
...
`game-store` now seeds the config Map with the player team (each player trains their selected
focus; "Balanced" players get the AIM team default), so the squad auto-trains weekly, bounded by
potential with fatigue as the brake. Scoped to the player team — the AI world's weekly growth is
carried by `processNaturalGrowth`, so the player still pulls ahead via training (the documented
intent) without rebalancing every AI roster. Regression test added (`training-processor.test.ts`).
