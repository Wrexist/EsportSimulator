...
async function main(){
 ...
 fs.writeFileSync(path.join(__dirname,'manifest.json'),JSON.stringify({method:'Built-in image_gen; locally resized under existing owner export authorization',kind:'Screenshot-based promotional composites, not unaltered gameplay screenshots',uploaded:false,exports:manifest},null,2));
 fs.writeFileSync(path.join(__dirname,'index.html'),`<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Esports Manager â€” new campaign</title><style>body{margin:0;background:#09141f;color:#f8fafb;font:16px/1.5 system-ui}main{max-width:1400px;margin:auto;padding:40px 24px}h1{font-size:48px;letter-spacing:-2px;margin:0}p{color:#aabccc}.grid{display:grid;grid-template-columns:1fr 1fr;gap:24px}article:first-child{grid-column:1/-1}article{background:#142334;border:1px solid #304358;border-radius:12px;overflow:hidden}img{width:100%;display:block}h2{font-size:20px;padding:0 20px}a{color:#5de9d2}footer{margin-top:32px}@media(max-width:800px){.grid{grid-template-columns:1fr}h1{font-size:32px}}</style><main><h1>Your club. Your next move.</h1><p>Eight screenshot-based promotional compositions with a coach overlay and gold trophy branding</p><p>For About This Game and advertising. Generated promotional compositions: interface text and details may differ from the actual game. Use the original captures for the Steam screenshot gallery.</p><div class="grid">${entries.map(e=>`<article><a href="promotional-1920x1080/${e.file}"><img src="promotional-1920x1080/${e.file}" alt="${e.title}"></a><h2>${e.title}</h2></article>`).join('')}</div><footer><a href="prompts.json">Prompt set</a> Â· <a href="manifest.json">Export validation</a><p>Not uploaded. No sales or download uplift has been measured.</p></footer></main></html>`);
