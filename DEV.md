# CLAUDE.md — EsportSimulator

CS:GO/CS2-style esports management sim. **Next.js 15 (app router) + React 19 + Zustand 5 (immer slices) + Electron 44**, shipping to **Steam**. All simulation runs client-side; saves persist to IndexedDB via `engine/save-manager.ts`.
...
- Branch: work stays on the designated `claude/*` branch; never push elsewhere; no PRs unless explicitly requested.
