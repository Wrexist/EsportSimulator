...
function Ground({ src }: { src: string }) {
    ...
    return (
        <group>
            {/* base board (sits just under the map for a framed-table edge) */}
            <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.4, 0]}>
                <planeGeometry args={[PLANE + 6, PLANE + 6]} />
    ...
    return (
        <group>
            ...
            <mesh position={[wx, 0.08, wz]} rotation={[-Math.PI / 2, 0, 0]}>
                ...
                <meshBasicMaterial color={eco} transparent opacity={0.75} side={THREE.DoubleSide} depthWrite={false} />
            </mesh>
            {/* soft ground shadow */}
            ...
            <mesh position={[wx, 4.6, wz]}>
                <sphereGeometry args={[1.7, 28, 28]} />
                <meshStandardMaterial color={color} emissive={color} emissiveIntensity={0.55} roughness={0.3} metalness={0.15} />
            </mesh>
        </group>
    )
}
        ...
        <group position={[wx, 0.16, wz]}>
            {[Math.PI / 4, -Math.PI / 4].map((r, i) => (
                <mesh key={i} rotation={[-Math.PI / 2, 0, r]}>
                    ...
                    <meshBasicMaterial color="#ff3333" transparent opacity={opacity} side={THREE.DoubleSide} depthWrite={false} />
                </mesh>
            ))}
        </group>
    )
...
function Scene({ radarSrc, dots, killLines, smokes, bombPosition, bombVisible, bombState, currentTime, autoRotate }: MapRadar3DProps & { autoRotate: boolean }) {
    ...
    return (
        <>
            ...
            <directionalLight position={[-40, 30, -25]} intensity={0.32} color="#88aaff" />
...
export default function MapRadar3D(props: MapRadar3DProps) {
    ...
    return (
        <WebGLErrorBoundary onError={props.onError}>
            <Canvas
                dpr={[1, 2]}
                camera={{ position: [0, 82, 74], fov: 42 }}
