...
38. **[Phase 7]** Deleted two confirmed-dead functions (zero callers): `EconomyManager.getPlayerBuy` V1 (~112 lines, superseded by the affordability-safe `getPlayerBuyV2`) and `TournamentManager.simulateWeeklyRegistrations` V1 (~80 lines, superseded by V2) (`engine/economy-manager.ts`, `engine/tournament-manager.ts`).
